...
const Header = ({ setDescripcion }) => {
    ...
    return (
        <>
            {/* Header fijo */}
            <header className="h-24 bg-gray-100 shadow-sm fixed top-0 left-0 w-full z-50">
                <div className="max-w-6xl mx-auto px-4 h-full">
                    <div className="flex justify-between items-center h-full">
                        {/* Logo principal */}
                        <Link to="/">
                            <div className={`w-44 relative z-10 ${isMenuOpen ? 'md:block hidden' : 'block'}`}>
                                <img
                                    src="/logoMartin.png"
                                    alt="Logo"
                                    className="w-full"
                        ...
                        {/* Navigation Menu */}
                        <nav
                            className={`fixed md:relative top-0 right-0 h-full w-72 md:w-auto
                        ...
                        >
                            ...
                            <div className="flex flex-col items-center w-full">
                                ...
                                <ul className={`w-full px-8 md:py-0 space-y-8 md:space-y-0 md:flex md:space-x-8 ${isMenuOpen ? "flex flex-col items-center" : "mt-[35px]"}`}>
                                    ...
                                    <li className={`${isMenuOpen ? "hover:bg-gray-300 rounded-md w-[150px] text-center p-1" : ""}`}>
                                        <Link
                                            to="/contacto"
                                            onClick={() => { closeMenu(); setDescripcion(''); }}
                                            className="block text-lg md:text-base font-medium text-gray-800 hover:text-black transition-colors duration-300"
                                        >
                                            Contacto
                                        </Link>
